...
use crate::workspaces::{blocks, plan};
use crate::{
...
/// # Errors
///
/// The first renderer's error, in call order below. Every step through
/// [`wallpaper::render_wallpaper`] is real. The last step, the display fragment, is real in
/// the one shape every scratch environment with no saved layout takes -- see
/// [`render_display_fragment`] -- and still reaches [`displays::render_displays`]'s stub,
/// [`RenderError::PortPending`], the moment `displays.toml` names an enabled display.
///
...
fn render_display_fragment(cx: &RenderCx<'_>) -> Result<(), RenderError> {
    let raw = blocks::load_toml(&cx.paths().host.displays)?;
    let has_displays = match raw.get("display") {
        None => false,
        Some(value) => !value
            .as_array()
            .ok_or_else(|| RenderError::Toml {
    ...
    if has_displays {
        return displays::render_displays(cx);
    }
...
mod tests {
    ...
    /// A `displays.toml` naming one enabled display reaches
    /// [`crate::displays::render_displays`]'s own stub -- the one remaining `PortPending`
    /// path through `render_all()`.
    #[test]
    fn render_all_reaches_the_render_displays_stub_once_a_display_is_saved() {
        let defaults = Defaults::compiled().expect("shipped defaults parse");
        ...
        let cx = RenderCx::new(defaults.values(), &paths, &monitors, &lua);
        let error =
            render_all(&cx, &Document::default()).expect_err("render_displays is still a stub");
        assert!(matches!(error, RenderError::PortPending("render_displays")));
        drop(std::fs::remove_dir_all(&paths.home));
    }
}
